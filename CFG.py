...
for i in range(0,Block[0]['end']):
	if nodes[i]['name']=='APICall':
		...
		if nodes[i]['value'].find('this.input')>=0:
			valname=nodes[i]['value'][nodes[i]['value'].find('(')+1:nodes[i]['value'].find(',')]
			#print(valname)
			device=nodes[i]['value'][nodes[i]['value'].find(',')+2:nodes[i]['value'].rfind(',')]
			#print(device)
			...
			if typee=='device':
				...
				api={'Gnum':0,'valname':valname,'device':'none','command':'input','type':device}
			source.append(api)
...
def TaintTrace(id):
	...
	while num>=Block[id['Gnum']]['start'] and num<=Block[id['Gnum']]['end']:
		if nodes[num]['name']=='Binary' or nodes[num]['name']=='Declaration':
			ifFind=0
			j=num+2
			#print(j)
			#ifLeftVal=0
...
for i in source:
	...
	print('\nSensitive Info: '+i['valname']+' , Source Device: '+i['device']+' , Source Api: '+i['command'])
	traveled.clear()
	DFSS(id,0)
