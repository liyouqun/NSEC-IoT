		...
		while nodes[j]['level']>lev:
			if nodes[j]['name']=='Constant':
				...
				if CallName in MethodNodeName:
					...
					#print(nodes[MethodCall[i]])
					break
			j=j+1
		if flag==1:
			while nodes[j]['level']>lev and j<=len(nodes)-3:
				j=j+1
			if nodes[j]['name']=='RightChild' or nodes[j]['name']=='EmptyStatement':
				k=j+1
				#print('before',k)
				while k<=len(nodes)-2 and nodes[k]['level']>=nodes[j]['level']:
					k=k+1
				#print('after',k)
				if nodes[k]['level']>3:
...
for i in range(0,len(nodes)-1):
	if nodes[i]['name']=='APICall':
		...
		if nodes[i]['value'].find('this.subscribe')>=0:
			#print(nodes[i]['value'])
			if(nodes[i]['value'].find('location')>=0)and 'location' not in InputVar:
				source.append({'Gnum':0,'valname':'location','device':'none','command':'subscribe','type':'location'})
				InputVar.append('location')
			if(nodes[i]['value'].find('app,')>=0):
				source.append({'Gnum':0,'valname':'app','device':'none','command':'subscribe','type':'app'})
				InputVar.append('app')
		else:
			if(nodes[i]['value'].find('location')>=0) and (nodes[i]['value'].find('this')<0):
				command=nodes[i]['value'][nodes[i]['value'].find('.')+1:nodes[i]['value'].rfind('(')]
				locationinfo={'Gnum':0,'valname':'location','device':'none','command':command,'type':'location'}
				if 'location' not in InputVar:
					source.append(locationinfo)
					InputVar.append('location')
	...
	#if nodes[i]['name']=='MethodNode' and nodes[i]['value']!='run':
		#break
		...
		if nodes[i]['name']=='APICall'and ifIfBlock(i)==0:
			...
			if callthis in SourceDevice or callthat in sourceAPI:
				...
				if nodes[j]['name']=='Binary' or nodes[j]['name']=='Declaration':
					...
					if id not in worklist and id not in done:
						source.append(api)
						#print(id)
						#source.append(id)
						worklist.append(id)
	...
	while num>=Block[id['Gnum']]['start'] and num<=Block[id['Gnum']]['end']:
		if (nodes[num]['name']=='Binary' or nodes[num]['name']=='Declaration') and ifIfBlock(num)==0:
				...
				if nodes[j]['name']=='Variable' and ifFind==0:
					...
					if valname==id['valname']:#找到右值
						...
						if valname_!=id['valname']:
							if ids not in worklist and ids not in done:
								worklist.append(ids)
			...
			if k_valname==id['valname']:
				m=k+1
				#print(ids)
				while m<=len(nodes)-1 and nodes[m]['level']>=nodes[k]['level']:
		...
		if nodes[num]['name']=='MethodCall' and nodes[num-1]['name']!='Binary' and nodes[num-1]['name']!='Declaration':
			...
			while nodes[i]['level']>nodes[num]['level'] and nodes[i]['level']<=len(nodes):
				if nodes[i]['name']=='Variable' and nodes[i]['level']>nodes[num]['level']:
					#print(i)
					count=count+1
		...
		if nodes[num]['name']=='APICall':
			...
			while nodes[i]['level']>nodes[num]['level']:
				if nodes[i]['name']=='Variable':
					#print(i)
					#print(nodes[i]['value'][nodes[i]['value'].find('-')+1:nodes[i]['value'].find(':')-1])
					#print(id['valname'])
					if nodes[i]['value'][nodes[i]['value'].find('-')+1:nodes[i]['value'].find(':')-1] == id['valname']:
						ifFind=1
						#print('****',id)
